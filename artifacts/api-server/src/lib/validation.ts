...
const cityIdSchema = z
  .number({ invalid_type_error: "Please select a city from the list" })
  .int()
  .positive();
...
const shopSchema = z.object({
  ...
  authorizedRep: z.literal(true, {
    errorMap: () => ({
      message: "You must confirm you are authorized to list this business",
    }),
  }),
  ...consentCheckboxes,
  honeypot: honeypotSchema,
...
export const submitSchema = z
  .discriminatedUnion("entityType", [personSchema, shopSchema, clubSchema])
...
export type PersonInput = Extract<SubmitInput, { entityType: "person" }>;
export type ShopInput = Extract<SubmitInput, { entityType: "shop" }>;
export type ClubInput = Extract<SubmitInput, { entityType: "club" }>;
...
export const AUTO_HIDE_REASONS = [
  "impersonation",
  "fake_business",
  "unauthorized_listing",
] as const;
